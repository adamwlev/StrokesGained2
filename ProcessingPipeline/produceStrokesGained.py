...
for year in range(2003,2017):
	...
	for i in range(score_min,score_max+1):
	    for j in range(1,i+1):
	        if i==j:
	            data.loc[(data.Hole_Score==i) & (data.Shot==j),'Strokes_Gained'] = data.loc[(data.Hole_Score==i) & (data.Shot==j),'Difficulty_Start'].values - 1
	        else:
	            data.loc[(data.Hole_Score==i) & (data.Shot==j),'Strokes_Gained'] = data.loc[(data.Hole_Score==i) & (data.Shot==j),'Difficulty_Start'].values - \
	                                                                               data.loc[(data.Hole_Score==i) & (data.Shot==j+1),'Difficulty_Start'].values - 1
